...
object Jwt {
    ...
    fun decodePayload(token: String): Map<String, Any?>? = try {
    ...
    fun isExpiring(token: String, skewSeconds: Long = 60): Boolean {
        val e = exp(token) ?: return true
        return e - (System.currentTimeMillis() / 1000) <= skewSeconds
    }

    private fun stringClaim(token: String, claim: String): String? =
        decodePayload(token)?.get(claim) as? String
}
