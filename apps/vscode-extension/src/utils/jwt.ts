...
//
// The extension never VERIFIES WorkOS access tokens locally (verification
// happens server-side against the WorkOS JWKS on the surviving vault routes and
// inside Convex). It only needs to READ two unverified claims from its own
// freshly obtained token: `exp` (to decide when to refresh) and `sid` (the
// WorkOS session id, recorded so the session shows up in the active-sessions
// UI).
//
...
/** Read the WorkOS user id (`sub` claim), or null. */
