...
describe("normalizeOrgRole", () => {
  ...
  it("falls the legacy project role manager through to developer", () => {
...
describe("fileProtectionMode", () => {
  ...
  it("assigned editor with hasWriteAccess → writable", () => {
    expect(
      fileProtectionMode({
        role: "editor",
        assigned: true,
  ...
  it("assigned viewer → strict-readonly (cannot request)", () => {
    expect(
      fileProtectionMode({
        role: "viewer",
        assigned: true,
