...
Each limit is scoped **per key** (or per CI/CD token). Two keys, or two tokens, never share a bucket.
...
See [Architecture](/architecture) for how the surfaces and the request loop fit together, and [API Security](/api-security) for the auth and audit model behind every request.
