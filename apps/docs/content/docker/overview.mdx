...
In Envpilot, go to **Organization → Settings → API Keys**. Create a key scoped to **one project** and the environment you are deploying, with the `variables` resource. Add `files` only if you need secret files — it is never granted by default.
...
The image reads the [public REST API](/api/overview), which is a Pro feature — the same gate the [GitHub Action](/action/overview) sits behind. A key scoped to a single project keeps the blast radius of a leaked container credential to that one project.
