...
The CLI and the VS Code extension are **human** surfaces: a person completes a WorkOS device-flow login in a browser, and their organization/project role decides what they can read and write. The REST API and the MCP server are **machine** surfaces: a program or an AI agent presents an API key. The GitHub Action is a machine surface too, but a deliberately narrow one — a CI reader locked to one project's variables and/or secure artifacts.
...
- **Scope** — projects (all or a list), environments (all or a list), and resources (`variables`, `accounts`, `projects`, `artifacts`, and optionally `requests`).
...
GitHub Action keys are the strict case: they are locked to a single project and may carry only the `variables` and `artifacts` resources. They can **never** file a request. CI reads; it does not negotiate.
