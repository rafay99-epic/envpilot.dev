...
Every key carries **surfaces** (`rest_api`, `mcp_server`, `github_action`) and a resource scope that can optionally include `requests` — the one machine write path, filing a variable request for a human to approve. See [Architecture](/architecture) for how the surfaces share one auth core, and [Rate limits](/rate-limits) for every per-key bucket.
