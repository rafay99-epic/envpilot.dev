...
- Every key is scoped along three dimensions: **projects** (all or a specific list), **environments** (all or a specific list), and **resources** (`variables`, `accounts`, `projects`, and optionally `requests`). A request for anything outside a key's scope is treated as if it doesn't exist.
