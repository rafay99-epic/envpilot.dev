...
A row whose key also exists in other projects shows **same key in N projects**. Its **Share across projects** action opens a sheet listing every project you can manage: projects holding the same value are preselected and their copies are moved into the shared row (they land in each project's trash for 30 days); projects with a different value are listed but cannot be picked until that is resolved. Pick an existing group or name a new one and confirm.
