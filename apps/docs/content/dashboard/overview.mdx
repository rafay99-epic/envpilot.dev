...
`/dashboard/workspaces` holds variables several projects share. One row, one rotation, no duplicates: link projects to a workspace and they read its variables on their own variables page (grouped as "From \<workspace\>", read-only) and in every pull — CLI, extension, API, GitHub Action.

Editing happens on the workspace page. Adoption pulls keys member projects already duplicate up into the workspace when their values are identical; differing values are reported and skipped.
