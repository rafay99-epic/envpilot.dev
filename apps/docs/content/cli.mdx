...
> Writes only the keys you have access to — denied keys are listed, never silently dropped. Push does not create approval requests; see [Push behavior by role](#push-behavior-by-role).
