...
A [workspace](/dashboard/organization) holds one copy of a variable that several projects share. Pulling a **project** returns its own rows plus everything inherited from the workspaces it belongs to — no parameter, no client change. There is deliberately no workspace endpoint: a workspace is not addressable by API keys, so pull from any project that reads it.
