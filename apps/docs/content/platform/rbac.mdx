...
<Callout type="info" title="Capabilities, not hardcoded branches">
  ...
  `org.members.invite`, `project.requests.review`, …) stored on the role.
  Nothing in the codebase asks "is this user an admin?" — it asks whether the
  role carries the capability. That is what makes custom roles possible at all.
</Callout>
