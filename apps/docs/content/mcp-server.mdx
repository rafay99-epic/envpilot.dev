...
| `That resource is not in this API key's scope`                                                 | The key is missing the resource a tool needs — edit the key's scopes (see the [Tools](#tools) table). The request tools append `— filing requests needs the "requests" resource`. |
