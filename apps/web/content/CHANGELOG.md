<!--
  Envpilot changelog. ONE file, newest entry first.

  Entries are separated by an "entry" HTML comment on its own line, and each
  carries YAML frontmatter validated by changelogFrontmatterSchema
  (apps/web/src/lib/changelog-schema.ts). A malformed entry is dropped and
  logged at build time, so a typo cannot silently disappear from the page.

  The body is MDX and renders through the same pipeline as the blog and
  docs: remark-gfm, rehype-pretty-code and the shared docsComponents.
  Anything valid in a blog post is valid here.

  To add a release: add a new block directly below this comment.
-->
...
The changelog no longer reads from the database. It is one markdown file in the repository, rendered at build time through the same pipeline as the blog and the documentation.

### What Changed
- Entries live in `apps/web/content/CHANGELOG.md`, newest first. Adding a release is a commit, not a database write
- The page renders with the shared documentation components, so headings, lists, tables, links and code blocks look identical to a blog post or a docs page
- The page is fully static. It no longer regenerates on a timer, which was the single largest source of hosting cost on the marketing site
- The hourly job that published scheduled entries is gone. A release goes live when its entry merges

### Elsewhere
- The uptime pill in the footer has been removed. The blog and the docs were fetching it cross-origin on every page load
- Public marketing pages no longer run the authentication middleware. They never read a session, so it was work with no result
- The release manifest at `/api/version` is now served from the CDN. Update checks and the minimum-version block are unchanged
...
A pass over the parts of the dashboard that had drifted away from the rest of it.
...
A project created from a template could come up with variables missing, with nothing on screen to say so. If you have created projects from templates in quick succession, it is worth checking them.
...
Picking a template used to send one request per variable from your browser, one after another, while you watched a spinner. A template with eight variables meant eight round trips before the page moved.
...
The Compare screen and the `envpilot diff` CLI command are gone. Variable history is untouched.
...
Creating an API key used to leave the Create button greyed out with no explanation when something was missing. The form now states its requirements up front and explains anything left undone.
...
The three settings screens were built at different times and had drifted apart. They now share one layout, one set of controls, and one set of rules.
...
Deleting a project now hides access immediately and queues a durable, permanent cleanup of the data and external secrets owned by that project.
...
Project, organization, member, and tag workflows now use authenticated Convex queries and mutations directly instead of bouncing routine dashboard work through an extra Next.js API layer.
...
The dashboard now wears the same design language as the redesigned landing page, and every colour in the product comes from one file instead of being spelled out in each component.
...
A round of fixes to the role system and to how documentation page limits are counted.
...
Documentation is how a project explains itself, and locking it behind Pro made the free plan worse at the one thing it exists to show. Project Documentation is now on every plan, bounded by page count rather than by a gate — and it answers to the same role system as everything else in the platform.
...
Envpilot solved half the secret problem. `DATABASE_URL` was covered; `upload.jks`, `id_ed25519`, `google-services.json` and `AuthKey.p8` were not — so they lived in Slack DMs, 1Password notes, and one teammate's laptop. Secret Files makes them first-class: uploaded from the dashboard, encrypted, and written to their exact path by the CLI and the VS Code extension.
...
The clipboard guard was supposed to stop secrets being copied out of managed `.env` files. Instead it disabled copy and cut in **every** file — TypeScript, C#, project files, anything — in every window, whether or not a project was linked. This release fixes that.
...
Envpilot can now send organization activity to the Slack and Discord channels where each team already works—without ever placing secret values in a message.
