...
export default defineConfig({
  ...
  // Parallel workers are safe because each worker mutates its OWN fixture
  // project (support.ts getWorkerProjectSlug) — they no longer contend on a
  // single shared variable list. CI pins 4 (ubuntu-latest has 4 vCPUs and
  // also runs the Next.js prod server); locally Playwright uses half the
  // CPU cores.
