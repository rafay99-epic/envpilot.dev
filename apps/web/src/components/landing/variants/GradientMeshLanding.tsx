  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-zinc-950 text-white">
      ...
      <div className="fixed inset-0 z-0">
        <MeshBlob className="h-[600px] w-[600px] bg-orange-600/15" style={{ top: "5%", left: "-5%" }} />
        <MeshBlob className="h-[500px] w-[500px] bg-amber-500/10" style={{ top: "40%", right: "-10%" }} />
        <MeshBlob className="h-[400px] w-[400px] bg-rose-600/10" style={{ bottom: "10%", left: "20%" }} />
        {/* Grain texture */}
        <div
      ...
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-zinc-950/70 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
          <Link href="/" className="flex items-center gap-2.5">
            ...
            <span className="text-base font-semibold tracking-tight">Envpilot</span>
          </Link>

          <nav className="hidden items-center gap-8 md:flex">
            {["Features", "Security", "Pricing"].map((item) => (
              <Link key={item} href={`#${item.toLowerCase()}`} className="text-sm text-zinc-500 hover:text-white transition-colors">
                {item}
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-4">
            <Link href="/sign-in" className="text-sm text-zinc-500 hover:text-white transition-colors">Log in</Link>
            <Link href="/sign-up" className="rounded-full bg-gradient-to-r from-orange-500 to-amber-500 px-5 py-2 text-sm font-medium transition-all hover:shadow-[0_0_20px_rgba(249,115,22,0.3)]">
              Start free
            </Link>
          </div>
        </div>
      ...
      <main className="relative z-10 pt-16">
        ...
        <section className="flex min-h-[85vh] items-center py-24">
          <div className="mx-auto max-w-6xl px-6">
            <motion.div initial="hidden" animate="visible" variants={stagger} className="max-w-3xl">
              <motion.div variants={fadeIn} className="mb-5 inline-flex items-center gap-2 rounded-full border border-orange-500/20 bg-orange-500/5 px-3.5 py-1 text-xs">
                <span className="h-1.5 w-1.5 rounded-full bg-orange-400" />
                <span className="text-orange-300/80">Trusted by 5,000+ engineering teams</span>
              </motion.div>

              <motion.h1 variants={fadeIn} className="text-5xl font-semibold tracking-tight leading-[1.1] md:text-6xl lg:text-7xl">
                Infrastructure for
                <br />
              ...
              <motion.p variants={fadeIn} className="mt-6 max-w-xl text-lg leading-relaxed text-zinc-400">
                Envpilot provides the encryption, access control, and developer
                tooling that teams need to manage environment variables at scale.
                From startup to enterprise.
              </motion.p>

              <motion.div variants={fadeIn} className="mt-8 flex flex-col gap-3 sm:flex-row">
                <Link href="/sign-up" className="group inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-orange-500 to-amber-500 px-6 py-3 text-sm font-medium transition-all hover:shadow-[0_0_30px_rgba(249,115,22,0.3)]">
                  Start building
                  <svg className="h-4 w-4 transition-transform group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                  </svg>
                </Link>
                <Link href="#features" className="inline-flex items-center rounded-full border border-zinc-800 px-6 py-3 text-sm text-zinc-400 hover:border-zinc-700 hover:text-white transition-all">
                  Read documentation
                </Link>
              </motion.div>

              <motion.div variants={fadeIn} className="mt-16 flex flex-wrap items-center gap-8">
                {["Vercel", "Stripe", "Linear", "Notion", "Figma"].map((co) => (
                  <span key={co} className="text-sm font-medium text-zinc-700">{co}</span>
                ))}
              </motion.div>
            </motion.div>
          </div>
          ...
          <div className="mx-auto max-w-6xl px-6">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeIn}>
              ...
              <p className="mt-3 max-w-lg text-zinc-500">
                Every feature designed around the principle that secrets should be
                encrypted, access-controlled, and fully auditable.
              </p>
            </motion.div>
        ...
        <section id="security" className="border-t border-white/5 py-24">
          <div className="mx-auto max-w-6xl px-6">
            ...
            >
              {[
                { step: "01", title: "Install", desc: "Add the CLI globally or the VS Code extension from the marketplace." },
                { step: "02", title: "Authenticate", desc: "Browser-based SSO login. No API keys to manage or rotate." },
                { step: "03", title: "Configure", desc: "Select your organization, project, and target environment." },
                { step: "04", title: "Sync", desc: "Pull variables to .env. Push changes back. Diff across environments." },
              ].map((s) => (
                <motion.div key={s.step} variants={fadeIn}>
                  <span className="text-3xl font-semibold text-zinc-800">{s.step}</span>
          ...
          <div className="mx-auto max-w-3xl px-6 text-center">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeIn}>
              ...
              <p className="mt-4 text-zinc-500">
                Free tier available. No credit card required. Scale when you&apos;re ready.
              </p>
              <div className="mt-8 flex items-center justify-center gap-3">
                <Link href="/sign-up" className="rounded-full bg-gradient-to-r from-orange-500 to-amber-500 px-6 py-3 text-sm font-medium transition-all hover:shadow-[0_0_30px_rgba(249,115,22,0.3)]">
                  Get started free
                </Link>
                <Link href="/sign-in" className="rounded-full border border-zinc-800 px-6 py-3 text-sm text-zinc-400 hover:border-zinc-700 transition-all">
                  Contact sales
                </Link>
              </div>
            </motion.div>
          ...
          <div className="flex flex-col items-center justify-between gap-4 sm:flex-row">
            ...
            <div className="flex gap-6">
              {["Privacy", "Terms", "Changelog", "Status"].map((l) => (
                <Link key={l} href={`/${l.toLowerCase()}`} className="text-xs text-zinc-600 hover:text-zinc-400 transition-colors">{l}</Link>
              ))}
            </div>
          </div>
        </div>
