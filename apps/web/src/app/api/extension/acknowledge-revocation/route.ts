...
export async function POST(request: NextRequest) {
  try {
    ...
    // Acknowledge the events. The token's owning user is resolved INSIDE
    // Convex (works even for revoked/expired tokens — that's the point of a
    // revocation ack), and the mutation verifies each event belongs to that
    // owner before clearing it, so events for other users are skipped.
    const result = await convex.mutation(
      api.permissionRevocationEvents.acknowledgeMultipleForToken,
      {
        accessToken,
