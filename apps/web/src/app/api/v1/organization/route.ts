 ...
 *
 * Public REST API v1 — org metadata + plan + bounded counts for the calling
 * API key's organization. Bearer-authenticated (`envpk_…`), no browser
 * session. Listed in proxy.ts unauthenticatedPaths. All authorization,
 * scope, gating, and rate limiting happens inside the Convex action
 * (convex/features/api/reads.ts:getOrganization), which routes through the
...
export async function GET(request: Request) {
  ...
  try {
    const result = await new ConvexHttpClient(convexUrl).action(
      api.features.api.reads.getOrganization,
      { token }
    );
    ...
    if (mapped.sentryFailureTag) {
      ...
      reportApiError(error, "GET /api/v1/organization");
    }
    return NextResponse.json(
