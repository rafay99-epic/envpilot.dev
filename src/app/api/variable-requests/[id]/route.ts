...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    if (!existingRequest) {
      return NextResponse.json({ error: 'Variable request not found' }, { status: 404 })
    }
    ...
    const message = error instanceof Error ? error.message : 'Failed to update variable request'

    if (message.includes('Not authorized') || message.includes('Only admins and team leads')) {
      return NextResponse.json({ error: message }, { status: 403 })
    }

    if (message.includes('already been') || message.includes('pending requests')) {
