...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (hasPendingDuplicate) {
      throw new Error("You already have a pending request for this variable key");
    }
...
export const review = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (membership.role !== "admin" && membership.role !== "team_lead") {
      throw new Error("Only admins and team leads can review variable requests");
    }
...
export const cancel = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (request.status !== "pending") {
      throw new Error(`Only pending requests can be canceled (current: ${request.status})`);
    }
