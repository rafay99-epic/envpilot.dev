 ...
 *
 * The device flow binds the freshly issued CLI tokens to the browser's
 * authenticated user. That user is now derived server-side from the verified
 * JWT identity (requireAuthedUser) — the pending cliSessions row is created
 * anonymously by `initiate`, and the acting user can no longer be supplied by
 * the client, closing the previous impersonation surface.
 *
 ...
 *
 * This inserts an ACTIVE token bound to the acting user. It is called only from
 * the trusted extension OAuth callback route
 * (apps/web/src/app/api/extension/auth/callback/route.ts) via an
 * authenticated Convex client, so the owning user is now derived server-side
 * from the verified JWT identity (requireAuthedUser) rather than a
 * client-supplied `userId`. The access/refresh tokens are still generated by
 * the route and passed in. Because ConvexHttpClient cannot invoke
 * internalMutation, this remains a public mutation.
 */
