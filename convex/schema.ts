...
export default defineSchema({
  ...
  // ==========================================
  pendingExtensionAuthSessions: defineTable({
    // Random token generated by the extension, used as the handshake key
    sessionToken: v.string(),
    // WorkOS user id (for Convex lookup on consume)
    workosUserId: v.string(),
    // Email (for display on the extension)
    email: v.string(),
    // Display name
    name: v.optional(v.string()),
    // The access/refresh tokens the extension will receive
    accessToken: v.string(),
    refreshToken: v.string(),
    // When the issued access token expires
    tokenExpiresAt: v.number(),
    // When this pending handshake record itself expires (10 min after creation)
    expiresAt: v.number(),
    // Created timestamp
    createdAt: v.number(),
  })
